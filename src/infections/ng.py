                ...
                for k in new_inf:
                    ...
                    if symptoms:
                        ...
                        test_delay = gamma.ppf(meta.at[j, 'treatment_threshold'], \
                                               inf_parameters['infection'].treatment_mean[0]/inf_parameters['infection'].treatment_var[0], \
...
def seek_treatment(pop_parameters, parameters, meta, partner_matrix, t):
    ...
    for age in [0, 1, 2, 3, 4]:
        for gender in [0, 1]:
            ...
            test_background = np.append(test_background, who_test[np.random.random(len(who_test)) <= background])
...
def initilise_infection_trackers(n_days):
    ...
    yt = pd.DataFrame(columns = ["S", 'E', "I", "R", 'T', 'V', "site0s", "site1s", "site2s", "site0a", "site1a", "site2a"])
...
def update_infection_trackers(t, pop_parameters, meta, inf_tracker):
    ...
    for age in [0, 1, 2, 3, 4]:
        for l2 in [0, 1]:
            temp1[t, ii] = np.sum(pop_parameters['lookup']['RA' + str(l2) + str(age)])
            temp2[t, ii] = np.sum(pop_parameters['lookup']['GA' + str(l2) + str(age)])
            ii = ii + 1
    ...
    for age in [0, 1, 2, 3, 4]:
        for gender in [0, 1]:
            for state in ['S', 'E', 'I', 'R', 'T', 'V']:
                temp[t, ii] = np.sum(pop_parameters['lookup']['S' + state + 'GA' + str(gender) + str(age)])
                ii = ii + 1
    ...
    for age in [0, 1, 2, 3, 4]:
        for risk in [0, 1]:
            for state in ['S', 'E', 'I', 'R', 'T', 'V']:
                temp[t, ii] = np.sum(pop_parameters['lookup']['S' + state + 'RA' + str(risk) + str(age)])
                ii = ii + 1
...
def make_infection_graphs(tt, inf_tracker, pop_parameters, save_loc = 'graphs/output/'):
    ...
    ax[1].plot(tt, yt["site0a"]/nt, linestyle = '--', label = "Rectal - asymptomatic", color = cmap(0))
    ax[1].plot(tt, yt["site1a"]/nt, linestyle = '--', label = "Urogenital - asymptomatic", color = cmap(1))
    ax[1].plot(tt, yt["site2a"]/nt, linestyle = '--', label = "Pharyngeal - asymptomatic", color = cmap(2))
    ...
    for age in [0, 1, 2, 3, 4]:
        for l2 in [0, 1]:
            ...
            ax[age, l2].plot(tt, np.divide(it[:, ii + 0], denom), label = 'Rectum')
            ax[age, l2].plot(tt, np.divide(it[:, ii + 3], denom), label = 'Rectum and Urethra')
            ax[age, l2].plot(tt, np.divide(it[:, ii + 6], denom), label = 'All Sites')
            ax[age, l2].plot(tt, np.divide(it[:, ii + 1], denom), label = 'Urethra')
            ax[age, l2].plot(tt, np.divide(it[:, ii + 4], denom), label = 'Rectum and Pharynx')
            ax[age, l2].plot(tt, np.divide(it[:, ii + 2], denom), label = 'Pharynx')
            ax[age, l2].plot(tt, np.divide(it[:, ii + 5], denom), label = 'Urethra and Pharynx')
            ii = ii + 7
            jj = jj + 1
    ...
    ax[4, 1].legend(loc = 'upper center', ncol = 3, bbox_to_anchor = (-0.1, -0.2), fancybox = True)
    ...
    for age in [0, 1, 2, 3, 4]:
        for l2 in [0, 1]:
            denom = inf_tracker['pop_size_age_gender'][:, jj]/100
            ax[age, l2].plot(tt, np.divide(it[:, ii + 0], denom), label = 'Rectum', color = cmap(0))
            ax[age, l2].plot(tt, np.divide(it[:, ii + 1], denom), label = 'Rectum - Asymptomatic', color = cmap(0), linestyle = '--')
            ax[age, l2].plot(tt, np.divide(it[:, ii + 2], denom), label = 'Pharynx', color = cmap(1))
            ax[age, l2].plot(tt, np.divide(it[:, ii + 3], denom), label = 'Pharynx - Asymptomatic', color = cmap(1), linestyle = '--')
            ax[age, l2].plot(tt, np.divide(it[:, ii + 4], denom), label = 'Urethra', color = cmap(2))
            ax[age, l2].plot(tt, np.divide(it[:, ii + 5], denom), label = 'Urethra - Asymptomatic', color = cmap(2), linestyle = '--')
            ax[age, l2].plot(tt, np.divide(it[:, ii + 6], denom), label = 'Co-infection', color = cmap(3))
            ax[age, l2].plot(tt, np.divide(it[:, ii + 7], denom), label = 'Co-infection - Asymptomatic', color = cmap(3), linestyle = '--')
            ii = ii + 8
            jj = jj + 1
    ...
    ax[0, 0].set_title('Female')
    ax[0, 1].set_title('Male')
    ax[4, 1].legend(loc = 'upper center', ncol = 4, bbox_to_anchor = (-0.1, -0.2), fancybox = True)
    
    # Save graph
    fig.savefig(save_loc + 'infections_prev_site_by_age_and_risk_and_symptoms.png', dpi = 200)
    plt.close(fig)
    ...
    fig.suptitle('Proportion of Sub-Populations Presenting for an STI Test\nBy Reason for Test')
    ax[4, 0]. set_xlabel('Day')
    ax[4, 1]. set_xlabel('Day')
    ax[0, 0].set_title('Female')
    ax[0, 1].set_title('Male')
    ax[4, 1].legend(loc = 'upper center', ncol = 5, bbox_to_anchor = (-0.1, -0.2), fancybox = True)
